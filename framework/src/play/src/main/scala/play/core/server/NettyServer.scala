...
class NettyServer(appProvider: ApplicationProvider, port: Option[Int], sslPort: Option[Int] = None, address: String = "0.0.0.0", val mode: Mode.Mode = Mode.Prod) extends Server with ServerWithStop {
  ...
  class PlayPipelineFactory(secure: Boolean = false) extends ChannelPipelineFactory {
    ...
    lazy val sslContext: Option[SSLContext] = //the sslContext should be reused on each connection
      Option(System.getProperty("play.http.sslcontextprovider")) match {
        case Some(className) =>
          (for {
            application <- appProvider.get
            classloader = application.classloader
            context <- ServerSSLContext.loadCustomSSLContext(className, classloader)
          } yield context) match {
            case Success(context) => Some(context)
            case Failure(e) =>
              Play.logger.error(s"cannot load custom SSL context from '$className' defined by 'play.http.sslcontextprovider'", e)
              None
          }
        case _ => {
